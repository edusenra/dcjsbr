	...
	async run(msg, args) {
		...
		if(this.client.shard) {
			try {
				...
				this.client.emit('warn', `Erro ao transmitir o comando para outros shards`);
				this.client.emit('error', err);
				await msg.reply(`comando \`${command.name}\` carregado, mas não foi possível carregar em outros shards.`);
				return null;
			}
		}

		await msg.reply(`comando \`${command.name}\` carregado${this.client.shard ? ' em todos shards' : ''}.`);
		return null;
	}
};
