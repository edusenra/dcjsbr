...
module.exports = class EvalCommand extends Command {
	constructor(client) {
		super(client, {
			...
			group: 'util',
			memberName: 'eval',
			description: 'Executa o código JavaScript.',
			details: 'Somente o(s) proprietário(s) do bot pode usar este comando.',
			ownerOnly: true,

			args: [
				{
					key: 'script',
					prompt: 'Qual código você gostaria de executar?',
					type: 'string'
				}
			]
		});
	...
	run(msg, args) {
		...
		const doReply = val => {
			if(val instanceof Error) {
				msg.reply(`erro de Callback: \`${val}\``);
			} else {
