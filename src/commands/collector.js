...
class ArgumentCollector {
	...
	constructor(client, args, promptLimit = Infinity) {
		if(!client) throw new TypeError('Um coletor para o cliente deve ser especificado.');
		if(!args || !Array.isArray(args)) throw new TypeError('Os argumentos do coletor devem ser um Array.');
		...
		for(let i = 0; i < args.length; i++) {
			if(hasInfinite) throw new Error('Nenhum outro argumento pode vir depois de um argumento infinito.');
			if(args[i].default !== null) hasOptional = true;
			else if(hasOptional) throw new Error('Argumentos obrigatórios não podem vir após argumentos opcionais.');
