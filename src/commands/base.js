...
class Command {
	...
	hasPermission(message, ownerOverride = true) {
		...
		if(this.ownerOnly && (ownerOverride || !this.client.isOwner(message.author))) {
			return `O comando \`${this.name}\` só pode ser usado pelo dono do bot.`;
		}
		...
		if(message.channel.type === 'text' && this.userPermissions) {
			...
			if(missing.length > 0) {
				if(missing.length === 1) {
					return `O comando \`${this.name}\` requer que você tenha a permissão "${permissions[missing[0]]}".`;
				}
				return oneLine`
					O comando \`${this.name}\` requer que você tenha as seguintes permissões:
	...
	async run(message, args, fromPattern) { // eslint-disable-line no-unused-vars, require-await
		throw new Error(`${this.constructor.name} não tem um método run ().`);
	}
	...
	setEnabledIn(guild, enabled) {
		...
		if(this.guarded) throw new Error('O comando é guardado.');
		if(!guild) {
	...
	unload() {
		...
		if(!require.cache[cmdPath]) throw new Error('Comando não pode ser descarregado.');
		delete require.cache[cmdPath];
	...
	static validateInfo(client, info) { // eslint-disable-line complexity
		...
		if(typeof info !== 'object') throw new TypeError('Informações de comando devem ser um Object.');
		...
		if(info.aliases && (!Array.isArray(info.aliases) || info.aliases.some(ali => typeof ali !== 'string'))) {
			throw new TypeError('Aliases de comando devem ser um array de strings.');
		}
		if(info.aliases && info.aliases.some(ali => ali !== ali.toLowerCase())) {
			throw new Error('Os aliases de comando devem estar em minúsculas.');
		}
		if(typeof info.group !== 'string') throw new TypeError('O grupo de comandos deve ser uma string.');
		if(info.group !== info.group.toLowerCase()) throw new Error('O grupo de comandos deve estar em minúsculas.');
		if(typeof info.memberName !== 'string') throw new TypeError('Nome do membro do comando deve ser uma string.');
		if(info.memberName !== info.memberName.toLowerCase()) throw new Error('Nome do membro do comando deve estar em minúsculas.');
		if(typeof info.description !== 'string') throw new TypeError('A descrição do comando deve ser uma string.');
		if('format' in info && typeof info.format !== 'string') throw new TypeError('Formato de comando deve ser uma string.');
		if('details' in info && typeof info.details !== 'string') throw new TypeError('Detalhes do comando devem ser uma string.');
		if(info.examples && (!Array.isArray(info.examples) || info.examples.some(ex => typeof ex !== 'string'))) {
			throw new TypeError('Exemplos de comando devem ser um array de strings.');
		}
		if(info.clientPermissions) {
			if(!Array.isArray(info.clientPermissions)) {
				throw new TypeError('O clientPermissions deve ser um array de chaves de permissão.');
			}
			for(const perm of info.clientPermissions) {
				if(!permissions[perm]) throw new RangeError(`O clientPermission: ${perm} inválido`);
			}
		}
		if(info.userPermissions) {
			if(!Array.isArray(info.userPermissions)) {
				throw new TypeError('O userPermissions deve ser um array de chaves de permissão..');
			}
			for(const perm of info.userPermissions) {
				if(!permissions[perm]) throw new RangeError(`O userPermission: ${perm} inválido`);
			}
		}
		if(info.throttling) {
			if(typeof info.throttling !== 'object') throw new TypeError('O throttling deve ser um Object.');
			if(typeof info.throttling.usages !== 'number' || isNaN(info.throttling.usages)) {
				throw new TypeError('Os usos de throttling devem ser um número.');
			}
			if(info.throttling.usages < 1) throw new RangeError('Os usos de throttling devem ser pelo menos 1.');
			if(typeof info.throttling.duration !== 'number' || isNaN(info.throttling.duration)) {
				throw new TypeError('A duração de throttling ser um número.');
			}
			if(info.throttling.duration < 1) throw new RangeError('A duração de throttling deve ser pelo menos 1.');
		}
		if(info.args && !Array.isArray(info.args)) throw new TypeError('Args devem ser um array.');
		if('argsPromptLimit' in info && typeof info.argsPromptLimit !== 'number') {
			throw new TypeError('O argsPromptLimit deve ser um número.');
		}
		if('argsPromptLimit' in info && info.argsPromptLimit < 0) {
			throw new RangeError('O argsPromptLimit deve ser pelo menos 0.');
		}
		if(info.argsType && !['single', 'multiple'].includes(info.argsType)) {
			throw new RangeError('O argsType deve ser "single" ou "multiple".');
		}
		if(info.argsType === 'multiple' && info.argsCount && info.argsCount < 2) {
			throw new RangeError('O argsCount deve ser pelo menos 2.');
		}
		if(info.patterns && (!Array.isArray(info.patterns) || info.patterns.some(pat => !(pat instanceof RegExp)))) {
			throw new TypeError('O padrões devem ser um Array de expressões regulares.');
		}
	}
}
