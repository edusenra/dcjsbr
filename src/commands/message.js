...
class CommandMessage {
	...
	parseArgs() {
		switch(this.command.argsType) {
			...
			default:
				throw new RangeError(`ArgsType desconhecido "${this.argsType}".`);
		}
	}
	...
	async run() { // eslint-disable-line complexity
		...
		if(this.command.guildOnly && !this.message.guild) {
			...
			return this.reply(`o comando \`${this.command.name}\` deve ser usado em um canal do servidor.`);
		}
		...
		if(this.message.channel.type === 'text' && this.command.clientPermissions) {
			...
			if(missing.length > 0) {
				...
				if(missing.length === 1) {
					return this.reply(
						`eu preciso da permissão "${permissions[missing[0]]}" para o comando \`${this.command.name}\` funcionar.`
					);
				}
		...
		if(throttle && throttle.usages + 1 > this.command.throttling.usages) {
			...
			return this.reply(
				`você não pode usar o comando \`${this.command.name}\` novamente por mais ${remaining.toFixed(1)} segundos.`
			);
		}
		...
		try {
			...
			if(!(retVal instanceof discord.Message || retVal instanceof Array || retVal === null || retVal === undefined)) {
				throw new TypeError(oneLine`
					o comando ${this.command.name}'s run() resolveu com um tipo desconhecido
					(${retVal !== null ? retVal && retVal.constructor ? retVal.constructor.name : typeof retVal : null}).
					Os run() dos comandos devem retornar uma Promise que resolva com uma mensagem, Array de mensagens ou null/undefined.
				`);
			}
			return retVal;
		} catch(err) {
			...
			if(err instanceof FriendlyError) {
				...
				return this.reply(stripIndents`
					ocorreu um erro durante a execução do comando: \`${err.name}: ${err.message}\`
					Você não deveria receber um erro como este.
					Por favor entre em contato com ${ownerList || 'o dono do bot'}${invite ? ` neste servidor: ${invite}` : '.'}
				`);
			}
		}
	}
	...
	respond({ type = 'reply', content, options, lang, fromEdit = false }) {
		...
		switch(type) {
			...
			case 'code':
				...
				return this.editCurrentResponse(channelIDOrDM(this.message.channel), { type, content, options });
			default:
				throw new RangeError(`Tipo de resposta desconhecida "${type}".`);
		}
	}
