...
class CommandRegistry {
	...
	registerGroups(groups) {
		if(!Array.isArray(groups)) throw new TypeError('Grupos devem ser um Array.');
		for(let group of groups) {
			...
			if(existing) {
				...
				this.client.emit('debug', `Grupo ${group.id} já está registrado; renomeado para "${group.name}".`);
			} else {
	...
	registerCommands(commands) {
		if(!Array.isArray(commands)) throw new TypeError('Comandos devem ser um Array.');
		for(let command of commands) {
			...
			if(!(command instanceof Command)) {
				this.client.emit('warn', `Tentando registrar um objeto de comando inválido: ${command}; pulando.`);
				continue;
			}
	 ...
	 */
	registerTypes(types) {
		if(!Array.isArray(types)) throw new TypeError('Tipos devem ser um Array.');
		for(let type of types) {
			...
			if(!(type instanceof ArgumentType)) {
				this.client.emit('warn', `Tentando registrar um objeto de tipo de argumento inválido: ${type}; pulando.`);
				continue;
			}
	...
	reregisterCommand(command, oldCommand) {
		if(typeof command === 'function') command = new command(this.client); // eslint-disable-line new-cap
		if(command.name !== oldCommand.name) throw new Error('O nome do comando não pode mudar.');
		if(command.groupID !== oldCommand.groupID) throw new Error('O grupo de comando não pode mudar.');
		if(command.memberName !== oldCommand.memberName) throw new Error('Comando memberName não pode mudar.');
		...
		this.client.emit('debug', `Comando registado ${command.groupID}:${command.memberName}.`);
	}
	...
	resolveGroup(group) {
		...
		throw new Error('Não é possível resolver o grupo.');
	}
	...
	resolveCommand(command) {
		...
		throw new Error('Não é possível resolver o comando.');
	}
