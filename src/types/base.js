...
class ArgumentType {
	...
	constructor(client, id) {
		...
		if(typeof id !== 'string') throw new Error('O tipo de argumento ID deve ser uma string.');
	...
	validate(value, msg, arg) { // eslint-disable-line no-unused-vars
		throw new Error(`${this.constructor.name} não tem um método validate().`);
	}
	...
	parse(value, msg, arg) { // eslint-disable-line no-unused-vars
		throw new Error(`${this.constructor.name} não tem um método parse().`);
	}
