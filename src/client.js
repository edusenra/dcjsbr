...
class CommandoClient extends discord.Client {
				...
				if(options.owner instanceof Array || options.owner instanceof Set) {
					for(const owner of options.owner) {
						this.fetchUser(owner).catch(err => {
							this.emit('warn', `Não é possível buscar o proprietário ${owner}.`);
							this.emit('error', err);
						});
					}
				} else {
					this.fetchUser(options.owner).catch(err => {
						this.emit('warn', `Não é possível buscar o proprietário ${options.owner}.`);
						this.emit('error', err);
					});
				}
			});
	...
	isOwner(user) {
		...
		if(!user) throw new RangeError('Não é possível resolver o usuário.');
		...
		throw new RangeError('A opção "owner" do cliente é um valor desconhecido.');
	}
