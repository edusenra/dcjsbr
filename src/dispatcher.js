...
class CommandDispatcher {
	 ...
	 * @example
	 * client.dispatcher.addInhibitor(msg => {
	 * 	if(!coolUsers.has(msg.author.id)) return ['cool', msg.reply('você não é legal o suficiente!')];
	 * });
	 */
				...
				if(cmdMsg.command) {
					...
					if(this.client.options.unknownCommandResponse) {
						responses = await cmdMsg.reply(
							`comando desconhecido. Use ${cmdMsg.anyUsage(
								'help',
	...
	buildCommandPattern(prefix) {
		...
		this.client.emit('debug', `Padrão de comando construído para prefixo "${prefix}": ${pattern}`);
		return pattern;
	}
}
